...
while (<STDIN>)
 {
  if($_ !~ /^"([^"]*?)"\t([0-9]{8}T[0-9]{6})\t([0-9]{8}T[0-9]{6})\t([0-9]{1,})\tCRC32:([0-9A-F]{8})\tMD5:([0-9a-f]{32})\tRIPEMD160:([0-9a-f]{40})\tSHA1:([0-9a-f]{40})\tSHA256:([0-9a-f]{64})\tSHA512:([0-9a-f]{128})\twhirlpool:([0-9a-f]{128})\t\n$/)
   {die($_);
   }
  else
   {
