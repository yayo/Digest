...
create table file
 (id INTEGER PRIMARY KEY AUTOINCREMENT, 
  ...
  UNIQUE(name,mtime),
  check(md5 is not null or ripemd160 is not null or sha1 is not null or sha256 is not null or sha512 is not null)
 );
...
create view loading_conflict as select * from loading where exists(select * from file where file.name=(select id from name where name.name=loading.name) and file.mtime=loading.mtime and (file.size<>loading.size or file.crc32<>(select id from crc32 where crc32.crc32=loading.crc32) or file.md5<>(select id from md5 where md5.md5=loading.md5) or file.ripemd160<>(select id from ripemd160 where ripemd160.ripemd160=loading.ripemd160) or file.sha1<>(select id from sha1 where sha1.sha1=loading.sha1) or file.sha256<>(select id from sha256 where sha256.sha256=loading.sha256) or file.sha512<>(select id from sha512 where sha512.sha512=loading.sha512)));
create view files as select file.id,name.name,mtime,ctime,size,hex(crc32.crc32) as crc32,hex(md5.md5) as md5,hex(ripemd160.ripemd160) as ripemd160,hex(sha1.sha1) as sha1,hex(sha256.sha256) as sha256,hex(sha512.sha512) as sha512 from file inner join name on file.name=name.id inner join crc32 on file.crc32=crc32.id inner join md5 on file.md5=md5.id inner join ripemd160 on file.ripemd160=ripemd160.id inner join sha1 on file.sha1=sha1.id inner join sha256 on file.sha256=sha256.id inner join sha512 on file.sha512=sha512.id;
create view crc32_collision as select * from file as f1 inner join file as f2 on f1.crc32=f2.crc32 and (f1.size<>f2.size or f1.md5<>f2.md5 or f1.ripemd160<>f2.ripemd160 or f1.sha1<>f2.sha1 or f1.sha256<>f2.sha256 or f1.sha512<>f2.sha512);
create view md5_collision as select * from file as f1 inner join file as f2 on f1.md5=f2.md5 and (f1.size<>f2.size or f1.crc32<>f2.crc32 or f1.ripemd160<>f2.ripemd160 or f1.sha1<>f2.sha1 or f1.sha256<>f2.sha256 or f1.sha512<>f2.sha512);
create view sha512_collision as select * from file as f1 inner join file as f2 on f1.sha512=f2.sha512 and (f1.size<>f2.size or f1.crc32<>f2.crc32 or f1.md5<>f2.md5 or f1.ripemd160<>f2.ripemd160 or f1.sha1<>f2.sha1 or f1.sha256<>f2.sha256);
create view incomplete as select * from file where crc32 is null or md5 is null or ripemd160 is null or sha1 is null or sha256 is null or sha512 is null;
create view duplicate_files_check_without_sha512_collision as select count(*),sha512,'{'||group_concat(name,',')||'}' from files group by sha512 having count(*) >1 order by 1 desc, 2 asc;
...
sqlite3 db "INSERT OR IGNORE into name(name) select name from loading;INSERT OR IGNORE into crc32(crc32) select crc32 from loading;INSERT OR IGNORE into md5(md5) select md5 from loading;INSERT OR IGNORE into ripemd160(ripemd160) select ripemd160 from loading;INSERT OR IGNORE into sha1(sha1) select sha1 from loading;INSERT OR IGNORE into sha256(sha256) select sha256 from loading;INSERT OR IGNORE into sha512(sha512) select sha512 from loading;"
sqlite3 db "INSERT OR IGNORE into file(name,mtime,ctime,size,crc32,md5,ripemd160,sha1,sha256,sha512) select name.id,loading.mtime,loading.ctime,loading.size,crc32.id,md5.id,ripemd160.id,sha1.id,sha256.id,sha512.id from loading inner join name on loading.name=name.name inner join crc32 on loading.crc32=crc32.crc32 inner join md5 on loading.md5=md5.md5 inner join ripemd160 on loading.ripemd160=ripemd160.ripemd160 inner join sha1 on loading.sha1=sha1.sha1 inner join sha256 on loading.sha256=sha256.sha256 inner join sha512 on loading.sha512=sha512.sha512;"
sqlite3 db "INSERT into load(prefix,memo) values('','');"
sqlite3 db "select max(id) from load;"
sqlite3 db "INSERT into loaded(load,file) select 1,file.id from loading inner join file on loading.mtime=file.mtime inner join name on file.name=name.id where loading.name=name.name;"
sqlite3 db "delete from loading;update sqlite_sequence set seq=0 where name='loading';"
sqlite3 db "select name.name,mtime,ctime,size,'CRC32:'||hex(crc32.crc32),'MD5:'||lower(hex(md5.md5)),'RIPEMD160:'||lower(hex(ripemd160.ripemd160)),'SHA1:'||lower(hex(sha1.sha1)),'SHA256:'||lower(hex(sha256.sha256)),'SHA512:'||lower(hex(sha512.sha512)) from file inner join name on file.name=name.id inner join crc32 on file.crc32=crc32.id inner join md5 on file.md5=md5.id inner join ripemd160 on file.ripemd160=ripemd160.id inner join sha1 on file.sha1=sha1.id inner join sha256 on file.sha256=sha256.id inner join sha512 on file.sha512=sha512.id where exists(select * from loaded where loaded.file=file.id and loaded.load=1 );" | sed -e "s/|/\t/g" -e "s/$/\t/" > tmp1
fc /b acc.md.txt tmp1

select count(*) from file where exists(select * from loaded where loaded.file=file.id and load=2);
select name.name,mtime,ctime,size,hex(crc32.crc32) as crc32,hex(md5.md5) as md5,hex(ripemd160.ripemd160) as ripemd160,hex(sha1.sha1) as sha1,hex(sha256.sha256) as sha256,hex(sha512.sha512) as sha512 from file inner join name on file.name=name.id inner join crc32 on file.crc32=crc32.id inner join md5 on file.md5=md5.id inner join ripemd160 on file.ripemd160=ripemd160.id inner join sha1 on file.sha1=sha1.id inner join sha256 on file.sha256=sha256.id inner join sha512 on file.sha512=sha512.id where exists(select * from loaded where loaded.file=file.id and load=2) and exists(select * from file as f2 where exists(select * from loaded where loaded.file=f2.id and load<>2) and f2.size=file.size and f2.crc32=file.crc32 and f2.md5=file.md5 and f2.ripemd160=file.ripemd160 and f2.sha1=file.sha1 and f2.sha256=file.sha256 and f2.sha512=file.sha512);
