 ...
 {struct STATE_S s;
  ...
  if(c)
   {off64_t readed;
    ...
    int f;
    off64_t fsize;
    assert((f=OPEN(p.c_str(),O_RDONLY|O_BINARY))!=-1);
 ...
 if(m)
  {static const std::set<std::string> ignore=
   ...
   if(ignore.end()==ignore.find(from))
    {static const boost::regex e("(?:SM3|SHA512|S1A384|SHA224|MD4|RIPEMD160|SHA256|SHAKE128|whirlpool|SHA3-256|SHAKE256|MDC2|SHA3-224|SHA3-512|SHA3-384|MD5|SHA1)");
...
int main(int argc, char *argv[])
 {boost::program_options::options_description desc;
  ...
  desc.add_options()
   ...
   ("digests,s", boost::program_options::value<std::string>(&s)->default_value(boost::algorithm::join(MDs,",")),"Digests selected to calculate");
  MDs.clear();
