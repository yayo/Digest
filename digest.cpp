...
int main(int argc, char *argv[])
 {boost::program_options::options_description desc;
  ...
  desc.add_options()
   ...
   //("digests,s", boost::program_options::value<std::string>(&s)->default_value("crc32,md5,sha1,sha256,sha512,ripemd160,whirlpool"),"Digests selected to calculate")
       ...
       {struct STATE_S s;
        ...
        if(c)
         {off64_t readed;
          ...
          int f;
          off64_t fsize;
          assert((f=OPEN(i->path().c_str(),O_RDONLY|O_BINARY))!=-1);
